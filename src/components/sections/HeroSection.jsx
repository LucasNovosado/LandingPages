...
import PopupWhatsapp from '../popups/PopupWhatsapp';
...
const HeroSection = ({ storeData }) => {
  ...
  const { logWhatsAppClick, logCallClick } = useGTM(storeData);
  
  const handleWhatsAppClick = () => {
    logWhatsAppClick();
    if (storeData?.popup_tipo === 'direct' && storeData?.link_whatsapp) {
        ...
        <div className={styles.grid}>
          <div className={styles.content}>
            <h1 className={styles.title}>
              Bateria Automotiva em {storeData?.cidade || 'Sua Cidade'}
            </h1>
            
            <div className={styles.priceBox}>
              {storeData?.preco_inicial && (
                <PriceTag 
                  price={storeData.preco_inicial} 
                  size="xl" 
                  showFrom={true}
                />
              )}
          ...
          <div className={styles.imageContainer}>
            {storeData?.imagem_produto ? (
              ...
              <div className="bg-gray-300 rounded-lg flex justify-center items-center" style={{ width: '400px', height: '300px' }}>
